...
export const Todi: CollectionConfig = {
  ...
  access: {
    ...
    update: ({ req: { user } }) => user?.role === 'admin' || user?.role === 'manager' || user?.role === 'user', 
    read: ({ req: { user } }) => user?.role === 'admin' || user?.role === 'manager' || user?.role === 'user',
  },
  hooks: {
    beforeChange: [
      async ({ data }) => {
  ...
  fields: [
    ...
    { name: 'final_cost', label: 'Final Cost', type: 'number' },
    {
      ...
      type: 'array',
      access: {
        update: ({ req }) => req?.user?.role === 'user' || req?.user?.role === 'admin', 
        create: ({ req }) => req?.user?.role === 'user' || req?.user?.role === 'admin',
        read: ({ req }) => req?.user?.role === 'admin' || req?.user?.role === 'manager' || req?.user?.role === 'user',
      },
      fields: [
        { name: 'date', label: 'Date', type: 'date', defaultValue: () => new Date() },
        { name: 'g_hydra_cost', label: 'Hydra Cost', type: 'number' },
        { name: 'g_truck_cost', label: 'Truck Cost', type: 'number' },
        { name: 'g_total_cost', label: 'Total Cost', type: 'number' },
        { name: 'g_total_block_area', label: 'Total Block Area', type: 'number' },

        {
          ...
          access: {
            ...
            create: ({ req }) => req?.user?.role === 'user' || req?.user?.role === 'admin',
            read: ({ req }) =>
              req?.user?.role === 'admin' || req?.user?.role === 'manager' || req?.user?.role === 'user',
          },
          fields: [
            {
              name: 'addmeasures',
