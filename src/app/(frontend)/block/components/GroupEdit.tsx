...
export default function GroupEdit({ todi, setTodi }: GroupProps) {
  ...
  const addBlock = (groupIndex: number) => {
    const updatedGroups = [...todi.group];
    updatedGroups[groupIndex].block.unshift({
      id: '',
      addmeasures: []
    });
    setTodi({ ...todi, group: updatedGroups });
  ...
  const addMeasure = (groupIndex: number, blockIndex: number) => {
    const updatedGroups = [...todi.group]
    updatedGroups[groupIndex].block[blockIndex].addmeasures.unshift({
      id: '',
      l: '',
      b: '',
      h: '',
    ...
    const updatedTodi = {
      ...
      g_total_cost: calculateTotalBlockCost(updatedGroups),
      g_total_block_area: calculateTotalBlockArea(updatedGroups)
    }
    setTodi(updatedTodi)
  }
  ...
  const handleNestedChange = (e: any, groupIdx: number, blockIdx?: number, measureIdx?: number) => {
    ...
    if (blockIdx === undefined) {
      ...
      if (name === 'g_hydra_cost' || name === 'g_truck_cost') {
        const totalBlockCost = updated[groupIdx].block.reduce((sum, block) => {
          return sum + block.addmeasures.reduce((blockSum, measure) => {
            return blockSum + (parseFloat(measure.block_cost) || 0)
          }, 0)
        }, 0)
        const totalBlockArea = updated[groupIdx].block.reduce((sum, block) => {
          return sum + block.addmeasures.reduce((blockSum, measure) => {
            return blockSum + (parseFloat(measure.block_area) || 0)
          }, 0)
        }, 0)
        updated[groupIdx].g_total_cost = totalBlockCost.toFixed(2)
        updated[groupIdx].g_total_block_area = totalBlockArea.toFixed(2)
      }
      ...
      {todi.group.map((group, gIdx) => (
        <div key={gIdx} className="border p-4 space-y-2 relative">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-5 gap-4">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-sm font-medium">Group Total Block Area </label>
              <input
                type="text"
                name="g_total_block_area"
                value={group.g_total_block_area}
                onChange={(e) => handleNestedChange(e, gIdx)}
                disabled
                className="p-2 border dark:bg-gray-700 dark:text-white bg-gray-100 text-gray-600"
              />
            </div>
          </div>
